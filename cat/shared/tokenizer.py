...
class PretrainedTokenizer(AbsTokenizer):
    def __init__(self, name: str, path: str) -> None:
        super().__init__()
        self._stats = OrderedDict({'name': name, 'path': path})
        self.load_state_dict(self._stats)
    ...
    def load_state_dict(self, state: OrderedDict):
        tokenizer_cls = getattr(transformers, state['name'])
        self.tokenizer = tokenizer_cls.from_pretrained(state['path'])
        self._stats = state

...
if [ $stage -le 1 ] && [ $stop_stage -ge 1 ]; then
    ...
    bash exp/rnnt-v1/run.sh --sta 1 --stop 1 || exit 1
fi

if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then

    bash exp/rnnt-v1/run.sh --sta 2 --stop 2 || exit 1
    ln -snf $(readlink -f exp/rnnt-v1/pkl) $dir/
fi
...
if [ $stage -le 4 ] && [ $stop_stage -ge 4 ]; then
    ...
    exec/e2edecode.sh $dir "test" $spmodel --check avg_best_10.pt --cer 

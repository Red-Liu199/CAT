...
if [ $stage -le 1 ] && [ $stop_stage -ge 1 ]; then
    for set in $trainset $devset; do
        ...
        cat $textdir/corpus.tmp | python3 exec/spm_encode.py --model=$spmdata/spm.model >$textdir/text_id.tmp || exit 1
...
if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
    ...
    python3 exec/transText2Bin.py --strip $textdir/${devset}.id $textdir/dev.pkl || exit 1

    python3 exec/transText2Bin.py --strip --concat 128 --nj $(nproc) $textdir/${trainset}.id $textdir/tr.pkl || exit 1
fi

...
if [ $stage -le 1 ] && [ $stop_stage -ge 1 ]; then
    ...
    bash exp/rnnt-v10/run.sh --sta 1 --sto 1 || exit 1
fi

if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then

    bash exp/rnnt-v10/run.sh --sta 2 --sto 2 || exit 1
    ln -s $(readlink -f exp/rnnt-v10/pkl) $dir/
fi
...
if [ $stage -le 3 ] && [ $stop_stage -ge 3 ]; then
    ...
    python3 rnnt/transducer_train.py --seed=1 \
        ...
        --grad-norm=5.0 \
        --amp \
        --resume=$dir/checks/checkpoint.046.pt \
        --checkall ||
        exit 1
fi

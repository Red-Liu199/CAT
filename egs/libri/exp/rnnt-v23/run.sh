...
if [ $stage -le 3 ] && [ $stop_stage -ge 3 ]; then
    ...
    python3 ctc-crf/transducer_train.py --seed=0 \
        ...
        --len-norm="L" \
        --databalance \
        --amp --resume=$dir/checks/checkpoint.082.pt \
        --checkall ||
        exit 1
fi
...
if [ $stage -le 4 ] && [ $stop_stage -ge 4 ]; then
    ...
    exec/e2edecode.sh $dir "dev_other:test_clean:test_other" $spmodel || exit 1

    python exec/avgcheckpoint.py --inputs=$dir/checks --num-best 10 || exit 1
    exec/e2edecode.sh $dir "dev_other:test_clean:test_other" $spmodel --check avg_best_10.pt
